...
const AssessmentPopup = ({ fetchAssessmentData, isOpen, onOutsideClick, assessment, linkExpiryDays, onCloseprofile }) => {
  console.log('assesview page props', assessment._id)
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const candidateAssessmentPromises = data.map(async (item) => {
          ...
          return {
            id: item._id,//scheduleassement id
            assessments: candidateResponse.data.candidateAssessments,//candidate assessment
          };
        });
  ...
  const [addedSections, setAddedSections] = useState(["fijeije"]);
  ...
  const onClickShareScheduledAssessmentCandidate = async (
    said,
    cid
  ) => {
    try {
      ...
      console.log("cid", cid),
      console.log("candidateAssessmentData[said]", candidateAssessmentData[said]),
      )
  ...
  const handleResend = () => {
    if (Object.keys(selectedCandidates).length === 0) return; // Prevent sending if no one is selected
  
    Object.keys(selectedCandidates).forEach((candidateId) => {
      const candidate = candidatesData.find((c) => c._id === candidateId);
      if (candidate) {
        onClickShareScheduledAssessmentCandidate(
          candidate.scheduledAssessmentId,
          candidate.candidateId._id
        );
      }
    });
  
    // Clear selected candidates after sending
    setSelectedCandidates({});
  };
          ...
          <div className="container mx-auto bg-white">
            <div className="mx-10">
                                  ...
                                  <div className="grid grid-cols-4 items-center p-2 gap-2">
                                    ...
                                    <div className="col-span-1 flex justify-end items-center">
                                      ...
                                      <p
                                        ...
                                        onClick={() =>
                                          onClickShareScheduledAssessmentCandidate(candidate.scheduledAssessmentId,
                                            candidate.candidateId._id
                                        )
                                        }
                                      >
              ...
              {activeTab === "Notifications" && (
                <Notification isassesmentProfileDetails={true} category="assessment" objectId={assessment._id} />
              )}
            </div>
          </div>
        )}
