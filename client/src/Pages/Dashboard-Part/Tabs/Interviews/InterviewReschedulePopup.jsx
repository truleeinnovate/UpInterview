...
const ReschedulePopup = ({ onClose, InterviewData }) => {
    const {
        fetchInterviewData,
    } = useCustomContext();
...
console.log(InterviewData)
    ...
    useEffect(() => {
        if (InterviewData && InterviewData.interview && InterviewData.interview.rounds.length > 0) {
            const round = InterviewData.interview.rounds[InterviewData.roundIndex];
    
            setDateTime(round.dateTime || "");
            setDuration(round.duration || "");
            setSelectedDate(round.dateTime?.split(" ")[0] || "");
            setStartTime(round.dateTime?.split(" ")[1] || "");
        }
    }, [InterviewData]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                status: "Reschedule",
                roundIndex: InterviewData.roundIndex,
                dateTime, 
                duration
            };
    
            const response = await axios.patch(
                `${process.env.REACT_APP_API_URL}/interview/reschedule/${InterviewData.interview._id}`, 
                payload
            );
