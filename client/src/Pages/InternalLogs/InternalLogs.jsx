...
export default function InternalLogs() {
  ...
  const fetchLogs = async () => {
    try {
      setLoading(true);
      const response = await internalLogService.getLogs(searchQuery, page, limit);
  ...
  const handleActionClick = (id) => {
    navigate(`/internal-logs-view-page/${id}`);
  };

  return (
    <>
      <div className="p-6">
        <h1 className="text-xl text-teal-600 font-bold">Internal Logs</h1>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className='bg-gray-50'>
            <tr>
              ...
              <th className="px-4 md:px-6 py-3 text-left text-xs md:text-sm font-medium text-teal-600 tracking-wider">Server Name</th>
              <th className="px-4 md:px-6 py-3 text-left text-xs md:text-sm font-medium text-teal-600 tracking-wider">Severity</th>
          ...
          <tbody>
            {loading ? (
              ...
              logs.map((log) => (
                <tr key={log.logId} className="border-b border-gray-200">
                  <td className="px-6 py-4 text-teal-600 cursor-pointer"onClick={() => handleActionClick(log.logId)}>{log.logId}</td>
                  ...
                  <td className="px-4 md:px-6 py-4 text-sm text-gray-700">{log.serverName}</td>
                  <td className="px-4 md:px-6 py-4 text-sm">
                    <span className={`${
