          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="mt-1">{formatCurrency(candidate.salary.current)}</p>
                  </div>
                  <div>
                    <span className="text-sm text-gray-500">Expected Salary</span>
                    <p className="mt-1">{formatCurrency(candidate.salary.expected)}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="mt-1">
                      <StatusBadge 
                        status={candidate.stage === 'offer' ? 'success' : 'pending'} 
                        text={candidate.stage.charAt(0).toUpperCase() + candidate.stage.slice(1)} 
                      />
                    </div>
                  </div>
                  <div>
              ...
              {candidate.interviews.length > 0 && (
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-4">Interviews</h3>
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {formatDate(interview.date)} with {interview.interviewer}
                            </p>
                          </div>
                          <StatusBadge status={interview.status} />
                        </div>
                        {interview.feedback && (
                          <p className="mt-2 text-sm text-gray-600">{interview.feedback}</p>
                        )}
                      </div>
                    ))}
                  </div>
                      ...
                      <div key={index} className="bg-gray-50 p-4 rounded-lg">
                        <div className="flex justify-between items-start">
                          ...
                          {assessment.score !== null ? (
                            ...
                            <StatusBadge status="pending" text={assessment.status} />
                          )}
                        </div>
                      </div>
                    ))}
