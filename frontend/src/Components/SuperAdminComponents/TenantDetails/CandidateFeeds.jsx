...
function CandidateFeeds({ parentId }) {
  const [feeds] = useState([
    ...
    },
    {
      ...
      metadata: {
        ...
        duration: "60 minutes",
        notes: "Please review candidate's technical assessment before the interview"
      }
    },
    {
      id: 7,
  ...
  const filteredFeeds = useMemo(() => {
    return feeds.filter(feed => {
      const matchesMainFilter = filter === 'all' || (filter === 'pinned' && feed.isPinned)
      const matchesTypeFilter = typeFilter === 'all' || feed.feedType === typeFilter
      ...
      case 'update':
        switch (action) {
          case 'assessment_completed':
            return <AiOutlineCheckCircle className="text-green-500" size={24} />
    ...
    switch (feed.action.name) {
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium">{metadata.initialData.position}</span>
              </div>
              <div className="flex items-center space-x-2">
                <AiOutlineLink className="text-gray-400" />
                <span className="text-gray-600">Source:</span>
                <span className="font-medium">{metadata.initialData.source}</span>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium">{metadata.initialData.referredBy}</span>
              </div>
              <div className="flex items-center space-x-2">
                <AiOutlineEnvironment className="text-gray-400" />
                <span className="text-gray-600">Department:</span>
                <span className="font-medium">{metadata.initialData.department}</span>
              </div>
            </div>
            <div className="mt-2 p-3 bg-white rounded-md border border-gray-200">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">Initial Status:</span>
                <StatusBadge status="pending" text={metadata.initialData.applicationStatus} />
              </div>
            </div>
          </div>
        )
      ...
      case 'candidate_updated':
        const getFieldIcon = (fieldName) => {
          switch (fieldName) {
            case 'status': return <AiOutlineTag className="text-gray-400" />
            case 'expectedSalary': return <AiOutlineDollar className="text-gray-400" />
            case 'email': return <AiOutlineMail className="text-gray-400" />
            case 'location': return <AiOutlineEnvironment className="text-gray-400" />
            case 'yearsOfExperience': return <AiOutlineStar className="text-gray-400" />
            case 'skills': return <AiOutlineTag className="text-gray-400" />
            default: return <AiOutlineEdit className="text-gray-400" />
          }
        }
        ...
        return (
          <div className="space-y-3">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-600">Changed {metadata.fieldName}:</span>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg p-4 border border-gray-200">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-2xl font-bold text-green-600">{metadata.score}%</span>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 flex justify-between text-sm text-gray-500">
                <span>Questions: {metadata.completedQuestions}/{metadata.totalQuestions}</span>
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            >
              <AiOutlineFilter />
              <span>Filter</span>
              <AiOutlineDown className={`transition-transform ${showFilters ? 'rotate-180' : ''}`} />
            </button>

            {showFilters && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border border-gray-200 z-10">
                <div className="p-2">
                  <div className="text-xs font-medium text-gray-500 mb-2">Feed Type</div>
                ...
                >
                  ...
                  {feed.isPinned && (
                    <div className="absolute right-2 top-2">
                      <AiOutlinePushpin className="text-yellow-500" />
                    </div>
                  )}
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex flex-wrap items-center mt-1 gap-2 text-sm text-gray-500">
                          <div className="flex items-center">
                            ...
                            <span className="truncate">{feed.metadata.changedBy}</span>
                          </div>
                          <span className="hidden sm:inline">•</span>
                          <time className="flex-shrink-0">{formatDate(feed.metadata.changedAt || feed.metadata.completedAt)}</time>
                        </div>
                      </div>
                      <StatusBadge 
                    ...
                    <div className={`${styles.text} bg-white bg-opacity-50 rounded-lg p-2 sm:p-4 overflow-x-auto`}>
