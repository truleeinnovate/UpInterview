...
const DragDropDashboard = ({
    ...
    try {
      ...
      if (itemType === "kpi") {
        ...
        if (dragIndex !== -1 && dragIndex !== targetIndex) {
          ...
          updatedLayout.kpiCards = items.map((item, index) => ({
            ...item,
            order: index,
          }));
        }
        ...
        if (dragIndex !== -1 && dragIndex !== targetIndex) {
          const [draggedItem] = items.splice(dragIndex, 1);
          items.splice(targetIndex, 0, draggedItem);
          updatedLayout.charts = items.map((item, index) => ({
            ...item,
            order: index,
          }));
        }
  ...
  const resetLayout = () => {
    ...
    const resetSettings = {
      ...customSettings,
      kpiCards: Object.keys(kpiCards).reduce(
        (acc, key) => ({ ...acc, [key]: true }),
        {}
      ),
      chartTypes: Object.keys(charts).reduce(
        (acc, key) => ({ ...acc, [key]: true }),
        {}
      ),
    };
