...
//         // console.log('Checking permission for:', permissionKey);
...
//         if (!isInitialized && !cachedPerms) {
//             // console.log('Not initialized and no cached permissions - denying');
//             return false;
//         }

//         if (loading) {
//             // console.log('Loading state - checking cached permissions');
//             if (!cachedPerms || !cachedPerms[permissionKey]) {
//                 console.log(`No cached permissions for ${permissionKey} - denying`);
//                 return false;
//             }

//             const hasPermission = cachedPerms[permissionKey]?.ViewTab || false;
//             // console.log(`Cached permission ${permissionKey}.ViewTab:`, hasPermission);
//             return hasPermission;
//         }

//         // console.log('Using checkPermission function');
//         const hasPermission = checkPermission(permissionKey);
//         // console.log(`checkPermission('${permissionKey}') returned:`, hasPermission);
//         return hasPermission;
//     };
...
//         closeAllDropdowns(dropdownState.requestsDropdown ? null : "requestsDropdown");
...
//                     permissionKey: "OrganizationRequest.ViewTab",
//                 },
//                 {
//                     path: "/withdrawal-request",
//                     label: "Withdrawal Request",
//                     permissionKey: "WithdrawalRequest.ViewTab",
//                 },
//                 {
//                     path: "/support-desk",
//                     label: "Support Desk",
...
//                                                 className={`flex items-center h-full px-3 ${isActive("/interviewer-requests") ||
//                                                     isActive("/outsource-interviewers") ||
//                                                     isActive("/organization-request") ||
//                                                     isActive("/withdrawal-request")
//                                                         ? 'text-custom-blue font-bold'
//                                                         : 'text-gray-600 hover:text-custom-blue'}`}
//                                             >
//                                                 Requests
...
//                                                                     ? 'bg-gray-100 text-custom-blue font-medium'
...
//                                             <NavLink
//                                                 to="/organization-request"
//                                                 className={`block px-4 py-3 rounded-md ${isActive("/organization-request")
//                                                     ? "bg-gray-100 text-custom-blue font-bold"
//                                                     : "text-gray-600 hover:bg-gray-100"
//                                                     }`}
//                                                 onClick={(e) => {
...
//                                             <NavLink
//                                                 to="/withdrawal-request"
//                                                 className={`block px-4 py-3 rounded-md ${isActive("/withdrawal-request")
//                                                     ? "bg-gray-100 text-custom-blue font-bold"
//                                                     : "text-gray-600 hover:bg-gray-100"
//                                                     }`}
//                                                 onClick={(e) => {
...
const CombinedNavbar = React.memo(() => {
    ...
    const notificationRef = useRef(null);
    const profileRef = useRef(null);
    const requestsRef = useRef(null); // Added for Requests dropdown
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            const refsToCheck =
                userType === 'superAdmin'
                    ? [moreRef, outlineRef, notificationRef, profileRef, requestsRef]
                    : [
                        assessmentRef,
                        interviewRef,
                        moreRef,
    ...
    const enhancedCheckPermission = (permissionKey) => {
        ...
        if (userType === 'superAdmin') {
            console.log(`SuperAdmin access granted for: ${permissionKey}`);
            return true;
        }
        ...
        // If the permission key already ends with .ViewTab, use it as is
        // Otherwise, check for .ViewTab in the cached permissions
        const basePermissionKey = permissionKey.endsWith('.ViewTab')
            ? permissionKey.replace('.ViewTab', '')
            : permissionKey;

        if (loading) {
            if (!cachedPerms || !cachedPerms[basePermissionKey]) {
                console.log(`No cached permissions for ${basePermissionKey} - denying`);
                return false;
            }
            // If the key already has .ViewTab, check directly, otherwise check for .ViewTab
            if (permissionKey.endsWith('.ViewTab')) {
                return !!cachedPerms[basePermissionKey];
            }
            return cachedPerms[basePermissionKey]?.ViewTab || false;
        }

        // For non-loading state, use the original checkPermission function
    ...
    const toggleAssessmentDropdown = () =>
        closeAllDropdowns(dropdownState.assessmentDropdown ? null : "assessmentDropdown");
    const toggleInterviewDropdown = () =>
        closeAllDropdowns(dropdownState.interviewDropdown ? null : "interviewDropdown");
    const toggleMoreDropdown = () =>
        closeAllDropdowns(dropdownState.moreDropdown ? null : "moreDropdown");
    const toggleRequestsDropdown = () =>
        closeAllDropdowns(dropdownState.requestsDropdown ? null : "requestsDropdown");
                                ...
                                {userType === "superAdmin" && (
                                    <>
                                        {enhancedCheckPermission("Tenants") && (
                                            <NavLink
                                                to="/tenants"
                                                className={`h-[52px] flex items-center relative ${isActive("/tenants")
                                                    ? "text-custom-blue font-bold"
                                                    : "text-gray-600 hover:text-custom-blue"
                                                    }`}
                                                onClick={(e) => {
                                        ...
                                        {enhancedCheckPermission("SupportDesk") && (
                                            <NavLink
                                                to="/support-desk"
                                                className={`h-[52px] flex items-center relative ${isActive("/support-desk")
                                                    ? "text-custom-blue font-bold"
                                                    : "text-gray-600 hover:text-custom-blue"
                                                    }`}
                                                onClick={(e) => {
                                        ...
                                        {enhancedCheckPermission("Billing") && (
                                            <NavLink
                                                to="/admin-billing"
                                                className={`h-[52px] flex items-center relative ${isActive("/admin-billing")
                                                    ? "text-custom-blue font-bold"
                                                    : "text-gray-600 hover:text-custom-blue"
                                                    }`}
                                                onClick={(e) => {
                                        ...
                                        <div className="relative flex items-center ml-6" ref={moreRef}>
                                            <button
                                                className={`h-[52px] flex items-center relative transition-colors duration-300 ${getMoreDropdownItems().some((item) => isActive(item.path))
                                                    ? "text-custom-blue font-bold"
                                                    : "text-gray-600 hover:text-custom-blue"
                                                    }`}
                                                onClick={toggleMoreDropdown}
                                            ...
                                            {dropdownState.moreDropdown && (
                                                <div className="absolute left-0 top-10 z-50 w-52 bg-white rounded-md shadow-lg border ring-black transform transition-all duration-300 ease-in-out origin-top p-2 pr-6">
                                                    <div className="flex flex-col min-w-[150px]">
                                                        {getMoreDropdownItems().map(({ path, label }) => (
                                                            <NavLink
                                                                key={path}
                                            ...
                                            enhancedCheckPermission("InterviewTemplates")) && (
                                                <div className="relative h-full flex items-center" ref={interviewRef}>
                                                    ...
                                                    {dropdownState.interviewDropdown && (
                                                        <div className="absolute top-full left-0 mt-0 z-50 w-48 rounded-md shadow-lg bg-white ring-1 p-2 ring-black ring-opacity-5 border">
                                                            <div className="space-y-1">
                                                                {getInterviewDropdownItems().map(({ path, label, permissionKey }) => (
                                                                    enhancedCheckPermission(permissionKey) && (
                                                                        <NavLink
                                                                            key={path}
                                                                            className={`block px-3 py-2 hover:bg-gray-100 hover:text-custom-blue rounded-md ${isActive(path)
                                                                                ? "bg-gray-100 text-custom-blue"
                                                                                : ""
                                                                                }`}
                                                                            to={path}
                                            ...
                                            enhancedCheckPermission("Assessments")) && (
                                                <div className="relative h-full flex items-center" ref={assessmentRef}>
                                                    ...
                                                    {dropdownState.assessmentDropdown && (
                                                        <div className="absolute top-full left-0 mt-0 z-50 w-48 rounded-md shadow-lg bg-white ring-1 p-2 ring-black ring-opacity-5 border">
                                                            <div className="space-y-1">
                                                                {getAssessmentDropdownItems().map(({ path, label, permissionKey }) => (
                                                                    enhancedCheckPermission(permissionKey) && (
                                                                        <NavLink
                                                                            key={path}
                                                                            className={`block px-3 py-2 whitespace-nowrap hover:bg-gray-100 hover:text-custom-blue rounded-md ${isActive(path)
                                                                                ? "bg-gray-100 text-custom-blue"
                                                                                : ""
                                                                                }`}
                                                                            to={path}
                                            ...
                                            enhancedCheckPermission("QuestionBank")) && (
                                                <div className="relative h-full flex items-center" ref={moreRef}>
                                                    ...
                                                    {dropdownState.moreDropdown && (
                                                        <div className="absolute top-full left-0 mt-0 z-50 w-48 rounded-md shadow-lg bg-white ring-1 p-2 ring-black ring-opacity-5 border">
                                                            <div className="space-y-1">
                                                                {getMoreDropdownItems().map(({ path, label, permissionKey }) => (
                                                                    enhancedCheckPermission(permissionKey) && (
                                                                        <NavLink
                                                                            key={path}
                                                                            className={`block px-3 py-2 hover:bg-gray-100 hover:text-custom-blue rounded-md ${isActive(path)
                            ...
                            <div className="p-4 space-y-1">
                                {userType === "superAdmin" && (
                                    <>
                                        ...
                                        {(enhancedCheckPermission("InterviewRequest") ||
                                            ...
                                            enhancedCheckPermission("WithdrawalRequest")) && (
                                                <div className="relative" ref={requestsRef}>
                                                    <button
                                                        className={`w-full text-left px-4 py-3 rounded-md flex justify-between items-center ${getRequestsDropdownItems().some((item) => isActive(item.path))
                                                            ? "bg-gray-100 text-custom-blue font-bold"
                                                            : "text-gray-600 hover:bg-gray-100"
                                                            }`}
                                                        onClick={toggleRequestsDropdown}
                                                    >
                                                        <span>Requests</span>
                                                        {dropdownState.requestsDropdown ? <IoIosArrowUp /> : <IoIosArrowDown />}
                                                    </button>
                                                    {dropdownState.requestsDropdown && (
                                                        <div className="mt-1 ml-4 space-y-1">
                                                            {getRequestsDropdownItems().map(({ path, label, permissionKey }) => (
                                        ...
                                        <div className="relative" ref={moreRef}>
                                            <button
                                                className={`w-full text-left px-4 py-3 rounded-md flex justify-between items-center ${getMoreDropdownItems().some((item) => isActive(item.path))
                                                    ? "bg-gray-100 text-custom-blue font-bold"
                                                    : "text-gray-600 hover:bg-gray-100"
                                                    }`}
                                                onClick={toggleMoreDropdown}
