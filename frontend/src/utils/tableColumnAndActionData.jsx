    ...
    },
    {
      ...
      render: (value) => (
        <div
          ...
          title={value
            ?.map((skill) => skill.skill || skill.SkillName)
            ?.join(", ")}
        >
    ...
    ? [
        {
          ...
          onClick: (row) =>
            callbacks.onStatusChange && callbacks.onStatusChange(row),
        },
        {
          key: "edit",
          label: "Edit",
      ...
      return (
        <div className="truncate max-w-[120px]">
          <div
            ...
            onClick={() =>
              callbacks.onViewPosition && callbacks.onViewPosition(position)
            }
...
export const getInterviewActions = (
  navigate,
  permissions = {},
  callbacks = {},
) => [
  ...
  return [
    {
      ...
      render: (value, row) => {
        ...
        let displayName = row?.contactDetails
        ...
        if (
          (row?.interviwer?.interviewer_type || row?.interviewer_type) ===
            "internal" &&
          row.contactId &&
    ...
    },
    {
      ...
      render: (value, row) => {
        let displayRole =
          row?.contactDetails?.currentRoleLabel ||
          row?.contactDetails?.currentRole ||
          "Interviewer";
        if (
          (row?.interviwer?.interviewer_type || row?.interviewer_type) ===
            "internal" &&
      ...
      ? [
          {
            key: "toggle_status",
            label: (row) =>
              row?.interviwer?.isActive ? "Deactivate" : "Activate",
            icon: (row) =>
              row?.interviwer?.isActive ? (
...
export const getInterviewTemplateColumns = (navigate, options = {}) => {
  ...
  return [
    ...
    },
    {
      ...
      render: (value) => (
        <span
          className="block text-gray-600 truncate max-w-[250px] cursor-default"
          title={value}
        >
    ...
    },
    {
      ...
      header: "Rounds",
      render: (value) => (
        <span className="text-gray-600">{value?.length || 0} Rounds</span>
      ),
    },
    {
      key: "bestFor",
  ...
  return [
    ...
    },
    {
      ...
      render: (value, row) => {
        ...
        return status ? (
          <StatusBadge
            status={status === "RequestSent" ? "Request Sent" : status}
          />
        ) : (
      ...
      ? [
          {
            ...
            show: (row) =>
              ![
                ...
                "FeedbackPending",
                "FeedbackSubmitted",
              ].includes(row?.rounds?.[0]?.status),

            onClick: (row) =>
              navigate(`/mock-interviews/${row._id}/edit`, {
                state: { from: "tableMode", isEdit: true },
              }),
          },
        ]
      : []),
...
export const getFeedbackColumns = (navigate, options = {}) => {
  ...
  return [
    ...
    },
    {
      ...
      header: "Date",
      render: (value, row) =>
        row.roundDetails?.dateTime?.split(" ")[0] || "N/A",
    },
    {
      key: "rating",
      header: "Rating",
    ...
    },
    {
      ...
      render: (value, row) => (
        <StatusBadge status={row.overallImpression?.recommendation} />
      ),
    },
  ];
};
  ...
  return [
    ...
    // }] : []),
    {
      ...
      show: (row) => {
        ...
        return (
          row.status === "draft" && row?.ownerId?._id === tokenPayload.userId
        );
      },

      onClick: (row) =>
        navigate(`/feedback/edit/${row._id}`, {
          state: { feedback: { ...row }, mode: "edit" },
        }),
    },
  ];
};
...
export const getCompanyColumns = (navigate, options = {}) => {
  ...
  return [
    ...
    },
    {
      ...
      render: (value) => (
        <span
          className="block text-gray-600 truncate max-w-[150px] cursor-default"
          title={capitalizeFirstLetter(value)}
        >
    ...
    },
    {
      ...
      render: (value) => (
        <span
          className="block text-gray-600 truncate max-w-[160px]  cursor-default"
          title={value}
        >
          {value || "-"}
        </span>
    ...
    },
    {
      ...
      header: "Status",
      render: (value) => (
        <StatusBadge status={capitalizeFirstLetter(value) || "Active"} />
      ),
    },
    {
      key: "createdAt",
...
export const getSupportTicketColumns = (navigate, options = {}) => {
  ...
  const getPath = (row) => {
    ...
    if (
      row.assignedToId === impersonatedUserId &&
      impersonatedRole === "Support_Team"
    ) {
      return `/support-desk/view/${row._id}`;
    }
    if (
      impersonatedRole === "Super_Admin" ||
      impersonatedRole === "Support_Team"
    ) {
      return `/support-desk/view/${row._id}`;
    }
