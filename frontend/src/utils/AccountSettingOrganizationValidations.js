...
// v1.0.1 - Ashok - Validations updated for phone
...
export const validateCompanyProfile = (formData) => {
  ...
  if (!safeTrim(formData.company)) errors.company = "Company name is required";
  if (!safeTrim(formData.industry)) errors.industry = "Industry is required";
  if (!safeTrim(formData.employees))
    errors.employees = "Company size is required";
  if (!safeTrim(formData.country)) errors.country = "Country is required";
  if (!safeTrim(formData.website)) errors.website = "Website is required";
  if (!safeTrim(formData.firstName))
    errors.firstName = "First name is required";
  if (!safeTrim(formData.lastName)) errors.lastName = "Last name is required";
  if (!safeTrim(formData.email)) errors.email = "Email is required";
  if (!safeTrim(formData.jobTitle)) errors.jobTitle = "Job title is required";
  if (!safeTrim(formData.phone)) errors.phone = "Phone number is required";
  ...
  if (
    ...
    !socialRegex.test(formData.socialMedia.linkedin)
  ) {
    errors.linkedin = "Invalid LinkedIn URL";
  }
  ...
  if (
    ...
    !socialRegex.test(formData.socialMedia.twitter)
  ) {
    errors.twitter = "Invalid Twitter URL";
  }
  ...
  if (
    ...
    !socialRegex.test(formData.socialMedia.facebook)
  ) {
    errors.facebook = "Invalid Facebook URL";
  }
  ...
  // Headquarters phone validation
  if (
    formData.headquarters?.phone &&
    !phoneRegex.test(formData.headquarters.phone)
  ) {
    errors.headquarters = {};
    errors.headquarters.phone = "Phone must be 10-15 digits";
  }

  // ✅ Regional offices phone validation (ARRAY)
  if (Array.isArray(formData.regionalOffices)) {
    const regionalErrors = [];

    formData.regionalOffices.forEach((office, index) => {
      const officeErrors = {};

      if (office.phone && !phoneRegex.test(office.phone)) {
        officeErrors.phone = "Phone must be 10-15 digits";
      }

      if (Object.keys(officeErrors).length > 0) {
        regionalErrors[index] = officeErrors;
      }
    });

    if (regionalErrors.length > 0) {
      errors.regionalOffices = regionalErrors;
    }
  }
