...
import { useUpdateVideoSettings } from "../../../../apiHooks/useUpdateVideoSettings";
// v1.0.0 --------------------------------------------------------------->
...
export function VideoCallingSettings() {
  ...
  const { mutateAsync: saveSettingsMutation, isPending } =
  ...
  // v1.0.1 - Fixed infinite re-render issue
  useEffect(() => {
    if (!settings || isOrganization === true) return;

    // Check if credentialType needs to be forced to "platform"
    if (settings.credentialType !== "platform") {
      setSettings((prev) => ({
        ...prev,
        credentialType: "platform",
      }));
    }
  }, [settings, isOrganization]); // Only re-run when isOrganization changes or settings is initially set
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      //   updateData
      // );
      const response = await saveSettingsMutation(updateData);
      ...
      notify.success(
        error.response?.message || "Failed to save settings. Please try again."
      );
    } finally {
      setSaving(false);
    }
  ...
  if (isError && !settings) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        <p className="text-red-800">Error loading settings: {isError}</p>
        <button
