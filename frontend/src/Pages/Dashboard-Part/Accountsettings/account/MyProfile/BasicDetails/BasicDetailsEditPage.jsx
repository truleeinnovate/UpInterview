...
const BasicDetailsEditPage = ({
  ...
  const handleFileChange = async (e) => {
    ...
    if (selectedFile) {
      ...
      setFile(selectedFile);
      setFilePreview(URL.createObjectURL(selectedFile));
      setIsProfileRemoved(false); // user selected a new file, so it's not "removed"
    }
  };
  ...
  useEffect(() => {
    ...
    // Use the profileData image path as initial preview (if any), and reset removal flag.
    setFilePreview(profileData?.imageData?.path || null);
    setIsProfileRemoved(false); // reset in case we're loading a new profile
    setSelectedCurrentRoleId(profileData?.roleId || "");
    setOriginalEmail(profileData.email || "");
    setOriginalProfileId(profileData.profileId || "");
    setStartDate(
  ...
  const roleOptions = (currentRole || []).map((r) => ({
    value: r._id,
    label: r.label,
  }));
  ...
  const roleOptionsWithCurrent = useMemo(() => {
    ...
    const currentLabel =
      formData.roleLabel || userProfile?.roleLabel || "Current Role";
  ...
  const handleRoleDropdownChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      roleId: val,
      roleLabel: roleOptions.find((o) => o.value === val)?.label || "",
    }));
    setErrors((prev) => ({ ...prev, roleId: "" }));
  };
    ...
    try {
      if (formData.email !== originalEmail) {
        ...
        if (res.data.success) {
          ...
          if (!updateId) {
            console.error("No valid ID for email update:", {
              from,
              profileData,
              resolvedId,
            });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-center items-center mb-4">
            <ProfilePhotoUpload
              ...
              selectedImage={displaySelectedImage}
              //   v1.0.2 --------------------------------------------------->
              fileError={fileError}
