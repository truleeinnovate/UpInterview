        ...
        <div className="border rounded-lg p-4 bg-gray-50">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="font-medium text-gray-900">{documents[type].fileName}</p>
                <p className="text-sm text-gray-500">
                  {documents[type].fileSize} • Uploaded {new Date(documents[type].uploadDate).toLocaleDateString()}
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              <button
                onClick={() => window.open(documents[type].url, '_blank')}
                className="p-2 text-gray-500 hover:text-gray-700 rounded-lg hover:bg-gray-200"
                title="View Document"
              >
                <EyeIcon className="h-5 w-5" />
              </button>
              <label className="p-2 text-custom-blue hover:text-custom-blue/80 rounded-lg hover:bg-blue-50 cursor-pointer" title="Replace Document">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className={`px-3 py-1 text-sm rounded-md ${
              documents?.coverLetter?.type === 'file' || !documents?.coverLetter?.type
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium text-gray-900">Cover Letter Text</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => {
                  ...
                  <div className="text-sm text-gray-600 max-h-32 overflow-y-auto">
                    {documents.coverLetter.text.split('\n').map((paragraph, index) => (
                      <p key={index} className="mb-2">{paragraph}</p>
                    ))}
                  </div>
                  <p className="text-xs text-gray-500 mt-2">
                    Last updated: {new Date(documents.coverLetter.uploadDate).toLocaleDateString()}
                  </p>
                </div>
              ) : (
                <div className="text-center py-8">
                  <DocumentTextIcon className="mx-auto h-12 w-12 text-gray-400" />
                  <p className="mt-2 text-gray-600">No cover letter text added</p>
                  <button
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-blue-50 border-l-4 border-custom-blue p-4 rounded-r-lg">
        <h4 className="text-sm font-medium text-custom-blue mb-2">Document Guidelines</h4>
