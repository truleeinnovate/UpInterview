  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(
        `${config.REACT_APP_API_URL}/emailTemplate/get-templates`
      );
        ...
        <div>
          {viewPage ? (
            <EmailTemplateViewPage
              show={viewPage}
              onClose={() => setViewPage(false)}
              record={filteredTemplates}
            />
          ) : (
            <div>
              <div className="flex justify-between items-center mb-4 px-4 py-2">
                <h2 className="text-base sm:text-lg md:text-lg lg:text-xl xl:text-xl 2xl:text-xl text-[#217989] font-semibold">
                  Email Templates
                </h2>
                <button
              ...
              <EmailTemplateDetails
                templates={filteredTemplates.slice(
