...
const SuggestedQuestionsComponent = ({
                    ...
                    category.id === 1
                        ? {
                            ...
                            options: category.options.map((opt) =>
                                opt.type.toLowerCase() === item ? { ...opt, isChecked: false } : opt
                            ),
                        }
                        : category
                )
                ...
                prev.map((category) =>
                    category.id === 2
                        ? {
                            ...category,
                            options: category.options.map((opt) =>
                                opt.level.toLowerCase() === item ? { ...opt, isChecked: false } : opt
                            ),
                        }
                        : category
                )
    ...
    return (
        <div className="h-full flex flex-col">
            {/* Search/Filter Bar */}
            <div className={`fixed flex items-center justify-between ${(type === "interviewerSection" || type === "assessment") ? "top-40 left-12 right-12" : "top-32 left-5 right-5"} z-50`}>
                <div>
                        ...
                        <div className="flex items-center">
                            ...
                            <Tooltip title="Next" enterDelay={300} leaveDelay={100} arrow>
                                <span
                                    ...
                                    className={`border p-2 text-xl rounded-md cursor-pointer ${currentPage === totalPages || totalPages === 0 ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-100"}`}
                                >
                                    <IoIosArrowForward />
                                </span>
                            </Tooltip>
