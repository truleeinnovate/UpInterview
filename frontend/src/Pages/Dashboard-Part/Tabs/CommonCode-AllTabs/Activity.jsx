...
function Activity({ parentId }) {
  ...
  useEffect(() => {
    const fetchFeeds = async () => {
      ...
      try {
        ...
        const fetchedFeeds = Array.isArray(response.data.data) ? response.data.data : [];
        setFeeds(fetchedFeeds);
        console.log('Set feeds:', fetchedFeeds);
        setError(null);
      } catch (err) {
        const errorMessage = err.response?.data?.message || err.message;
        console.error('Error fetching feeds:', errorMessage, err.response?.data);
  ...
  const filteredFeeds = useMemo(() => {
    return feeds.filter((feed) => typeFilter === 'all' || feed.feedType === typeFilter);
  }, [feeds, typeFilter]);

  const getFeedIcon = (type, action) => {
    switch (type) {
      case 'info':
        if (action.name === 'candidate_created' || action.name === 'position_created') {
  ...
  const formatValue = (fieldName, value) => {
    ...
    switch (fieldName) {
      ...
      case 'skills':
        return Array.isArray(value) ? value.map((s) => s.skill || s).join(', ') : value;
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {Object.entries(metadata)
                .filter(([key]) => !['tenantId', 'ownerId', 'CreatedBy', 'LastModifiedById'].includes(key))
                .slice(0, 4)
                .map(([key, value]) => (
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium">{new Date(metadata.scheduledFor).toLocaleString()}</span>
              </div>
              <div className="flex items-center space-x-2">
                    ...
                    {['all', 'info', 'alert', 'update'].map((type) => (
                      <button
                        ...
                        className={`w-full text-left px-2 py-1 rounded text-sm ${
                          typeFilter === type ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-50'
                        }`}
                        onClick={() => {
                          setTypeFilter(type);
                          setShowFilters(false);
      ...
      <div className="relative">
        ...
        <div className="space-y-6">
          {loading ? (
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-2">
                      <div className="min-w-0 flex-1">
                        <h4 className={`font-medium ${styles.text} truncate`}>{feed.action.description}</h4>
                        <div className="flex flex-wrap items-center mt-1 gap-2 text-sm text-gray-500">
                          <div className="flex items-center">
                            ...
                            <span className="truncate">{feed.metadata.changedBy || 'System'}</span>
                          </div>
                          <span className="hidden sm:inline">•</span>
                          <time className="flex-shrink-0">
                            {formatDate(feed.metadata.changedAt || feed.createdAt)}
                          </time>
                        </div>
                      </div>
                      <StatusBadge status={feed.feedType} text={feed.feedType.toUpperCase()} />
                    </div>
            ...
            <div className="text-center py-8 text-gray-500">No activity found for the selected filters</div>
          )}
        </div>
      </div>
    </div>
