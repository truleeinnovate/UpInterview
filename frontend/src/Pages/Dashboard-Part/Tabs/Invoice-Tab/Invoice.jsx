...
const InvoiceTab = () => {
  ...
  const fetchInvoiceData = useCallback(async () => {
    ...
    try {
      ...
      const Invoice_res = await axios.get(
        `${config.REACT_APP_API_URL}/get-invoice-id/${ownerId}`
      );
  ...
  const FilteredData = () => {
    ...
    return sortedData.filter((invoice) => {
      const fieldsToSearch = [invoice.invoiceNumber, invoice.status];
          ...
          <motion.div className="lg:flex xl:flex 2xl:flex items-center lg:justify-between xl:justify-between 2xl:justify-between md:float-end sm:float-end mb-4">
                ...
                <span onClick={handleListViewClick}>
                  <List
                    className={`text-xl cursor-pointer mr-4 ${
                      viewMode === "table" ? "text-custom-blue" : ""
                    }`}
                  />
                </span>
              </Tooltip>
            ...
            <div className="flex items-center">
              ...
              <div className="flex">
                ...
                >
                  <span
                    className={`border p-2 mr-2 text-lg sm:text-md md:text-md rounded-md ${
                      currentPage === 0
                        ? " cursor-not-allowed"
                        : "cursor-pointer"
                    }`}
                ...
                <Tooltip title="Next" enterDelay={300} leaveDelay={100} arrow>
                  <span
                    className={`border p-2 text-lg sm:text-md md:text-md rounded-md ${
