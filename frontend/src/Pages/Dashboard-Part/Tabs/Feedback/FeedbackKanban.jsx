...
import { User, Calendar, Eye, Edit, CheckCircle, Clock, XCircle, AlertCircle } from 'lucide-react';
                  ...
                  <div className="bg-gray-100 rounded-lg p-3">
                    ...
                    <div className="space-y-2">
                      {getColumnItems(column).map((item) => (
                        <div key={item._id} className="bg-white rounded-lg p-3 shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs text-gray-500">
                              ...
                              <span className="truncate">{item.interviewerId?.firstName + ' ' + item.interviewerId?.lastName}</span>
                            </span>
                            <span className="text-xs text-gray-500">
                          ...
                          <div className="flex items-center justify-between">
                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full border ${getRecommendationColor(item.overallImpression?.recommendation)}`}>
                              {item.overallImpression?.recommendation}
                            </span>
                          </div>
                          {item.generalComments && (
                            <p className="text-xs text-gray-600 mt-2 truncate">{item.generalComments}</p>
                          )}
