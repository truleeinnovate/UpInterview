 ...
 // <-------------------------------v1.0.0
  ...
  return (
    <div className={`flex ${isFullscreen ? 'flex-col' : 'flex-col'} gap-6 h-full min-h-screen p-6`}>
      ...
      {/* Main Content */}
      <div className={`flex ${isFullscreen ? 'flex-row w-full' : 'flex-col'} gap-6 flex-1`}>
        {/* Left Panel - Candidate Details */}
        <div className={`${isFullscreen ? 'w-1/3' : 'w-full'} bg-white rounded-lg shadow-sm border border-gray-200 h-fit`}>
          <div className="p-6">
            {/* Candidate Profile */}
            <div className="flex flex-col items-center mb-6">
              ...
              <p className="text-sm text-gray-600 text-center">{candidate.email}</p>
              <p className="text-xs text-gray-500 mt-1">{candidate.experience} year(s) experience</p>
            </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{assessment.NumberOfQuestions}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">{isEachSection ? '-' : assessment.passScore}</p>
                  </div>
                </div>
              </div>
        ...
        <div className={`${isFullscreen ? 'w-2/3' : 'w-full'} bg-white rounded-lg shadow-sm border border-gray-200 flex-1 h-fit`}>
    ...
    // <-------------------------------v1.0.1
              ...
              return (
                <div key={section.sectionName || index} className="last:border-b-0">
                  ...
                  >
                    <div>
                      <h4 className="font-medium text-gray-800 text-sm">{section.sectionName}</h4>
                      <div className="flex flex-wrap gap-x-4 gap-y-2 mt-2 text-xs text-gray-600">
                        <span>Questions: {answerCount}/{section.questions?.length || 0}</span>
                        <span>Score: {candidateSection?.totalScore || 0}{isEachSection && `/${candidateSection?.passScore || 0}`}</span>
                        {isEachSection && (
                          <span className={`font-medium ${
                            sectionPass ? 'text-green-600' : 'text-red-600'
                          }`}>
                  ...
                  {toggleStates[index] && (
                    <div className="p-4 space-y-4">
                      {section.questions?.map((question, qIndex) => {
                        const answer = candidateSection?.Answers.find(ans => 
                        ...
                        return (
                          <div key={question._id} className="p-4 border border-gray-200 rounded-lg text-sm">
                            <div className="flex items-start">
                              ...
                              <div className="flex-1">
                                <h5 className="font-medium text-gray-800 mb-3">
                                  {question.snapshot?.questionText || 'Question not available'}
                                </h5>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3">
                                  <div>
                                    ...
                                    <p className="font-medium">
                                      {question.snapshot?.correctAnswer || 'N/A'}
                                    </p>
                                  </div>
                                  <div>
