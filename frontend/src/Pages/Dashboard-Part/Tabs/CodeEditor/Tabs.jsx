...
export default function Tabs({ tabs }) {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#0a0e27] relative overflow-hidden">
      ...
      <div className="relative z-20 flex items-center justify-between px-6 py-4 bg-slate-900/80 backdrop-blur-xl border-b border-slate-700/50 shadow-2xl">
          ...
          <div className="flex gap-2">
            {tabs.map((tab, index) => (
              <button
                ...
                // className={`group relative px-6 py-1.5 font-bold transition-all duration-300 flex items-center gap-3 rounded-md overflow-hidden ${
                //   activeTab === index
                //     ? "bg-gradient-to-r from-custom-blue via-custom-blue/90 to-custom-blue/60 text-white shadow-xl shadow-cyan-500/30 scale-105"
                //     : "text-slate-400 hover:text-slate-200 hover:bg-slate-800/50"
                // }`}
              >
                {activeTab === index && (
