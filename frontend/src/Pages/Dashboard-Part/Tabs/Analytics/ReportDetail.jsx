...
import { ArrowLeft, Calendar, Filter, Download, BarChart3, Table, Eye, Settings } from 'lucide-react';
...
const ReportDetail = () => {
      ...
      case 'interview':
        return {
          ...
          columns: [
            ...
            { key: 'interviewerType', label: 'Type', render: (value) => getStatusBadge(value) }
          ]
        };
      case 'interviewer':
        return {
          ...
          columns: [
            { key: 'name', label: 'Name' },
            { key: 'type', label: 'Type', render: (value) => getStatusBadge(value) },
            { key: 'specialization', label: 'Specialization' },
            { key: 'totalInterviews', label: 'Total Interviews' },
            { key: 'rating', label: 'Rating' },
            { key: 'skills', label: 'Skills', render: (value) => value.join(', ') }
          ]
        };
      case 'assessment':
        return {
  ...
  const getDashboardKPIs = () => {
    ...
    switch (report.type) {
      case 'interview':
        return [
          { title: 'Total Interviews', value: reportData.length, icon: Calendar },
          { title: 'Completed', value: reportData.filter(i => i.status === 'completed').length, icon: Eye },
          { title: 'Scheduled', value: reportData.filter(i => i.status === 'scheduled').length, icon: Calendar },
          { title: 'Average Score', value: (reportData.filter(i => i.score > 0).reduce((sum, i) => sum + i.score, 0) / reportData.filter(i => i.score > 0).length || 0).toFixed(1), icon: BarChart3 }
        ];
      case 'interviewer':
        return [
          { title: 'Total Interviewers', value: reportData.length, icon: Calendar },
          { title: 'Internal', value: reportData.filter(i => i.type === 'internal').length, icon: Eye },
          { title: 'External', value: reportData.filter(i => i.type === 'external').length, icon: Calendar },
          { title: 'Average Rating', value: (reportData.reduce((sum, i) => sum + i.rating, 0) / reportData.length || 0).toFixed(1), icon: BarChart3 }
        ];
      case 'assessment':
        return [
          { title: 'Total Assessments', value: reportData.length, icon: Calendar },
          { title: 'Passed', value: reportData.filter(a => a.status === 'passed').length, icon: Eye },
          { title: 'Failed', value: reportData.filter(a => a.status === 'failed').length, icon: Calendar },
          { title: 'Average Score', value: (reportData.reduce((sum, a) => sum + a.score, 0) / reportData.length || 0).toFixed(1), icon: BarChart3 }
        ];
      case 'candidate':
        return [
          { title: 'Total Candidates', value: reportData.length, icon: Calendar },
          { title: 'Active', value: reportData.filter(c => c.status === 'active').length, icon: Eye },
          { title: 'In Final Stage', value: reportData.filter(c => c.stage === 'final_interview' || c.stage === 'offer_extended').length, icon: Calendar },
          { title: 'Average Experience', value: Math.round(reportData.reduce((sum, c) => sum + parseInt(c.experience), 0) / reportData.length || 0) + ' years', icon: BarChart3 }
        ];
      default:
        return [];
    }
      ...
      {activeView === 'dashboard' ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <InterviewerUtilizationChart data={chartData.interviewerUtilization || []} />
