...
const ReportDetail = () => {
  ...
  // Prevent layout jumps by letting the top header and filters stay visible
  // Loading skeletons are now handled individually inside the main content area
      ...
      {isLoading ? (
        activeView === "dashboard" ? (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 text-lg font-medium text-gray-900">
              <div className="h-6 w-48 bg-gray-200 rounded shimmer"></div>
            </div>
            <div className="p-6 space-y-4">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="h-12 w-full bg-gray-100 rounded shimmer"></div>
              ))}
            </div>
          </div>
        )
