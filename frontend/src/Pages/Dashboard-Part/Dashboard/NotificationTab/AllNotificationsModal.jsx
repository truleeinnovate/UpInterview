...
const AllNotificationsModal = ({ isOpen, onClose, notifications }) => {
  ...
  const filterNotifications = () => {
    ...
    if (activeTab === 'email') {
      filtered = filtered.filter(notification => notification.type === 'email');
    } else if (activeTab === 'whatsapp') {
      filtered = filtered.filter(notification => notification.type === 'whatsapp');
    }
    ...
    if (selectedFilter !== 'all') {
      filtered = filtered.filter(notification => notification.status.toLowerCase() === selectedFilter.toLowerCase());
    }
                      ...
                      {notification.type === 'email' && (
                        <div className="flex flex-col space-y-1 text-xs text-gray-500">
                          <div className="flex items-center space-x-2">
                            ...
                            <span>{Array.isArray(notification.recipients) ? notification.recipients.join(', ') : notification.recipients}</span>
                          </div>
