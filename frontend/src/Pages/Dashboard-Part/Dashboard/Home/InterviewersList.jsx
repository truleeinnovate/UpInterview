...
const InterviewersList = () => {
  ...
  console.log('interviewers in ho me', interviewers);
  ...
  const formattedInterviewers = interviewersList.map((availability) => {
    const contact = availability.contact || {};
    const nextSlot = availability.days?.[0]?.timeSlots?.[0]; // Get first available slot for nextAvailable
    return {
      id: availability._id,
      ...
      rating: 4.5, // Placeholder (not in backend data)
      department: contact.industry || 'N/A',
      company: contact.industry || 'N/A',
      location: contact.location || 'N/A',
      completedInterviews: 0, // Placeholder (not in backend data)
      ...
      availability: nextSlot ? 'Available' : 'Unavailable' // Based on time slots
    };
  });
    ...
    <div className="bg-white/80 backdrop-blur-lg p-6 rounded-2xl shadow-sm border border-gray-200 hover:shadow-lg transition-all duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-1 gap-4 max-h-[calc(100vh-400px)] overflow-y-auto pr-2 -mr-2">
        {formattedInterviewers.length === 0 ? (
          ...
          formattedInterviewers.map((interviewer) => (
            <div
              key={interviewer.id}
              className="p-4 border border-gray-100 rounded-xl hover:border-indigo-100 hover:bg-indigo-50/50 transition-all duration-300"
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    ...
                    <div className="flex items-center space-x-1.5 text-xs text-gray-500">
                      <Calendar size={14} />
                      <span>{interviewer.nextAvailable}</span>
                    </div>
                  </div>
