...
const Kanban = ({ integrationLogs, loading }) => {
        ...
        <div className="h-full">
          ...
          >
            ...
            <span className="px-3 py-1.5 bg-white rounded-lg text-sm font-medium text-gray-600 shadow-sm border border-gray-200">
              {integrationLogs?.length || 0} integration logs
            </span>
          </motion.div>
            ...
            {integrationLogs?.length === 0 ? (
              <div className="col-span-full flex flex-col items-center justify-center py-16 text-gray-500">
                ...
                <p className="text-gray-500 text-center max-w-md">
                  There are no integration logs to display at the moment. Create a new
                  position to get started.
                </p>
              </div>
            ) : (
              integrationLogs?.map((integration, index) => (
                <motion.div
                  key={integration._id}
                  ...
                  whileHover={{ y: -5 }}
                  onClick={() =>
                    navigate(`/integration/view-details/${integration._id}`, {
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center">
                      ...
                      <div>
                        <h4 className="text-sm font-medium text-custom-blue">
                          {/* {integration?.interviewerNo || "N/A"} */}
                          integration #{integration?._id?.slice(-5, -1) || "N/A"}
                        </h4>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      ...
                      <motion.button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(`/integration/edit-integration/${integration._id}`);
                        }}
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex items-center gap-1.5 text-gray-600">
                        ...
                        {formatCreatedDate(integration?.createdAt) || "N/A"}
                      </div>
                      <div className="flex items-center gap-1.5 text-gray-600">
                        <MapPin className="w-4 h-4 text-red-500" />
                        <span>{integration?.Location || "Not disclosed"}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4">
                    <div className="flex flex-wrap gap-1">
                      {integration?.candidateId?.skills
                        ?.slice(0, 3)
                        .map((skill, index) => (
                          <motion.span
                            key={index}
