...
const getPriceForCycle = (plan, cycle) => {
  const row = Array.isArray(plan?.pricing) ? plan.pricing.find((p) => p.billingCycle === cycle) : null;
  if (!row) return '—';
...
export default function Plans() {
  ...
  // Data hook
  const { plans, isLoading, isError, error, createPlan, updatePlan, deletePlan, isMutating } =
  ...
  const columns = [
    ...
    { key: 'maxUsers', header: 'Max Users' },
    {
      ...
      header: 'Active',
      render: (val, row) => (
        <StatusBadge status={row.active ? 'active' : 'inactive'} text={row.active ? 'Active' : 'Inactive'} />
      ),
    },
    {
      key: 'createdAt',
  ...
  return (
    ...
    <div className="p-4">
      ...
      <Toolbar
        ...
        onNextPage={() => setCurrentPage((p) => (p + 1 >= totalPages ? p : p + 1))}
                ...
                {['organization', 'individual'].map((type) => (
                  <label key={type} className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={() => {
                        const exists = tempFilters.subscriptionTypes.includes(type);
                        const next = exists
                          ? tempFilters.subscriptionTypes.filter((t) => t !== type)
                          : [...tempFilters.subscriptionTypes, type];
                        setTempFilters((prev) => ({ ...prev, subscriptionTypes: next }));
                      }}
                ...
                {['Active', 'Inactive'].map((status) => (
                  <label key={status} className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={() => {
                        const exists = tempFilters.activeStates.includes(status);
                        const next = exists
                          ? tempFilters.activeStates.filter((s) => s !== status)
                          : [...tempFilters.activeStates, status];
                        setTempFilters((prev) => ({ ...prev, activeStates: next }));
                      }}
                      className="h-4 w-4 rounded accent-custom-blue focus:ring-custom-blue"
                    />
                ...
                {[
                  ...
                  <label key={opt.key} className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={() => setTempFilters((prev) => ({ ...prev, createdDate: opt.key }))}
                      className="h-4 w-4 rounded accent-custom-blue focus:ring-custom-blue"
                    />
      ...
      {view === 'table' ? (
        <TableView
          ...
          actions={actions}
          emptyState={isError ? (error?.message || 'Failed to load plans') : 'No plans found.'}
          autoHeight={false}
        />
      ) : (
        <PlanKanbanView
      ...
      <ConfirmationPopup
        ...
        title="Delete Plan"
        message={`Are you sure you want to delete "${planToDelete?.name || planToDelete?.planId } plan"?`}
