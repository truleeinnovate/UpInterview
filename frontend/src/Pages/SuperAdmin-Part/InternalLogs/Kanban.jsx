...
const Kanban = ({ internalLogs, loading }) => {
        ...
        <div className="h-full">
          ...
          >
            ...
            <span className="px-3 py-1.5 bg-white rounded-lg text-sm font-medium text-gray-600 shadow-sm border border-gray-200">
              {internalLogs?.length || 0} internal logs
            </span>
          </motion.div>
            ...
            {internalLogs?.length === 0 ? (
              <div className="col-span-full flex flex-col items-center justify-center py-16 text-gray-500">
                ...
                <h3 className="text-lg font-medium text-gray-700 mb-2">
                  No internalLog logs Found
                </h3>
                <p className="text-gray-500 text-center max-w-md">
                  There are no internal logs to display at the moment.
                  Create a new position to get started.
                </p>
              </div>
            ) : (
              internalLogs?.map((internalLog, index) => (
                <motion.div
                  key={internalLog._id}
                  ...
                  whileHover={{ y: -5 }}
                  onClick={() =>
                    navigate(`/internalLog/view-details/${internalLog._id}`, {
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        <h4 className="text-sm font-medium text-custom-blue">
                          {/* {internalLog?.interviewerNo || "N/A"} */}
                          internal log #
                          {internalLog?._id?.slice(-5, -1) || "N/A"}
                        </h4>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      <motion.button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(
                            `/internalLog/view-details/${internalLog._id}`,
                            {
                              state: { from: location.pathname },
                            }
                          );
                      ...
                      <motion.button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(
                            `/internalLog/edit-internalLog/${internalLog._id}`
                          );
                        }}
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex items-center gap-1.5 text-gray-600">
                        ...
                        {formatCreatedDate(internalLog?.createdAt) || "N/A"}
                      </div>
                      <div className="flex items-center gap-1.5 text-gray-600">
                        <MapPin className="w-4 h-4 text-red-500" />
                        <span>{internalLog?.Location || "Not disclosed"}</span>
                      </div>
                    </div>
                  </div>
