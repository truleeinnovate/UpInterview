...
function RatesTableView({ filterCategory, onEdit }) {
  ...
  const filteredRateCards = filterCategory === 'all' 
    ? rateCards 
  ...
  const getRateRangeDisplay = (levels, currency) => {
    ...
    return `${formatCurrency(minRate, currency)} - ${formatCurrency(maxRate, currency)}`
  }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4 gap-4">
        <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="text-xl font-semibold">{filteredRateCards.length}</div>
        </div>
        <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="text-xl font-semibold">
            {Math.round(filteredRateCards.reduce((sum, card) => sum + card.discountMockInterview, 0) / filteredRateCards.length)}%
          </div>
        </div>
      </div>
