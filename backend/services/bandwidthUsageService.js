 ...
 */
async function updateBandwidthUsage(
  tenantId,
  ownerId,
  sizeInBytes,
  operation = "unknown"
  ...
  try {
    if (!tenantId) {
      console.warn(
        "[BANDWIDTH] No tenantId provided, skipping bandwidth update"
      );
    ...
    if (!usage) {
      console.error(
        "[BANDWIDTH] No usage document found for tenant:",
        tenantId
      );
    ...
    if (bandwidthIndex === -1) {
      console.warn(
        "[BANDWIDTH] No User Bandwidth attribute found in usage document"
      );
      return {
        success: false,
        message: "No User Bandwidth usage attribute found",
      };
    }
    ...
    return {
      ...
      usage: {
        ...
        percentage:
          currentAttr.entitled > 0
            ? Math.min((newUtilized / currentAttr.entitled) * 100, 100)
            : 0,
      },
    };
  } catch (error) {
  ...
  try {
    ...
    if (!bandwidthAttr) {
      ...
      return {
        canUse: true,
        remaining: Infinity,
        message: "Bandwidth tracking not configured",
      };
    }
    ...
    if (sizeInGB > 0 && remaining < sizeInGB) {
      return {
        canUse: false,
        remaining,
        message: `Insufficient bandwidth. Required: ${sizeInGB.toFixed(
          2
        )} GB, Available: ${remaining.toFixed(2)} GB`,
        usage: {
          utilized,
          entitled,
          remaining,
          percentage: Math.min((utilized / entitled) * 100, 100),
        },
      };
    }
    ...
    return {
      canUse: false,
      remaining: 0,
      message: "Error checking bandwidth usage",
    };
  }
}
...
async function getBandwidthUsageStats(tenantId, ownerId) {
  try {
    ...
    const remaining = Math.max(0, entitled - utilized);
    const percentage =
      entitled > 0 ? Math.min((utilized / entitled) * 100, 100) : 0;
