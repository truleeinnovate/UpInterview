...
// ──────────────────────────────────────────────────────────
// AGENDA-BASED SCHEDULING COMMENTED OUT FOR DEV
// In dev, the roundNoShowCron.js handles NoShow detection
// instead of Agenda. Uncomment this when deploying to Azure.
// ──────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────
// DEV MODE: scheduleOrRescheduleNoShow is a no-op
// The cron job (roundNoShowCron.js) handles NoShow detection
// ──────────────────────────────────────────────────────────
async function scheduleOrRescheduleNoShow(roundDoc, options = {}) {
  console.log("[NoShow] scheduleOrRescheduleNoShow called (DEV MODE - no-op, cron handles NoShow)");
  console.log("[NoShow] Round ID:", roundDoc._id, "| Status:", roundDoc.status, "| DateTime:", roundDoc.dateTime);
  // In dev mode, the roundNoShowCron.js periodically checks for
  // Scheduled/Rescheduled rounds where interview time + 5 min has passed
  // and processes NoShow automatically. No agenda scheduling needed.
  return;
}

// ──────────────────────────────────────────────────────────
// ORIGINAL AGENDA-BASED CODE (COMMENTED OUT FOR DEV)
// Uncomment when deploying to Azure where Agenda works
// ──────────────────────────────────────────────────────────
...
module.exports = { scheduleOrRescheduleNoShow, extractStartDate, NO_SHOW_DELAY_MINUTES };
