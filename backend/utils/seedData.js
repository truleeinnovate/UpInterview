...
const seedData = async () => {
  try {
    ...
    for (const tenant of createdTenants) {
      templateTypes.forEach((template, index) => {
        reportTemplates.push({
          tenantId: tenant.tenantId,
          templateId: `RPT_${tenant.tenantId}_${String(index + 1).padStart(3, '0')}`,
          name: template.name,
    ...
    const createdReportTemplates = await ReportTemplate.insertMany(reportTemplates);
    console.log(`✅ Created ${createdReportTemplates.length} report templates`);
    ...
    console.log(`   Interviewers: ${createdInterviewers.length}`);
    ...
    console.log(`   Report Templates: ${createdReportTemplates.length}`);
