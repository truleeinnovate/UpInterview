...
const trackUploadBandwidth = async (req, res, next) => {
  try {
    ...
    const ownerId =
      req.body?.ownerId || req.query?.ownerId || req.headers["x-user-id"];

    if (!tenantId) {
      console.warn(
        "[BANDWIDTH] No tenantId found, skipping bandwidth tracking"
      );
      return next();
    }
    ...
    const checkResult = await checkBandwidthUsageLimit(
      tenantId,
      ownerId,
      totalSizeInBytes
    );
    ...
    const result = await updateBandwidthUsage(
      tenantId,
      ownerId,
      totalSizeInBytes,
      "upload"
...
const trackDownloadBandwidth = async (req, res, next) => {
  try {
    ...
    const tenantId =
      req.query?.tenantId || req.headers["x-tenant-id"] || req.body?.tenantId;
    const ownerId =
      req.query?.ownerId || req.headers["x-user-id"] || req.body?.ownerId;
