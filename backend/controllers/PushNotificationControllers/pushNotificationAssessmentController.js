...
async function getUserDetails(userId) {
  try {
    ...
    console.error(
      "[ASSESSMENT NOTIFICATIONS] Error fetching user details:",
      error
    );
    return null;
  }
...
async function getCandidateDetails(candidateId) {
  try {
    ...
    console.error(
      "[ASSESSMENT NOTIFICATIONS] Error fetching candidate details:",
      error
    );
    return null;
  }
  ...
  try {
    ...
    const notification = new PushNotification({
      ...
      message: `New assessment "${
        assessment.AssessmentTitle || assessment.order || "Assessment"
      }" has been created by ${creatorName} (ID: ${
        assessment.AssessmentCode ||
        assessment.scheduledAssessmentCode ||
        assessment._id
      })`,
      type: "system",
      ...
      metadata: {
        ...
        assessmentCode:
          assessment.AssessmentCode || assessment.scheduledAssessmentCode,
    ...
    console.error(
      "[ASSESSMENT NOTIFICATIONS] Error creating assessment creation notification:",
      error
    );
  }
}
...
async function createAssessmentScheduledNotification(
  scheduledAssessment,
  candidateIds = []
) {
  try {
    ...
    const creatorName = creator
      ? `${creator.firstName} ${creator.lastName}`
      : "System";
    const expiryDate = moment(scheduledAssessment.expiryAt).format(
      "MMMM Do, YYYY, h:mm a"
    );
    ...
    console.error(
      "[ASSESSMENT NOTIFICATIONS] Error creating assessment scheduling notification:",
      error
    );
  }
}
...
async function createAssessmentStatusUpdateNotification(
  assessment,
  oldStatus,
  newStatus
) {
  try {
    ...
    console.error(
      "[ASSESSMENT NOTIFICATIONS] Error creating status update notification:",
      error
    );
  }
}
...
async function createAssessmentSubmissionNotification(candidateAssessment) {
  try {
    ...
    const candidate = await getCandidateDetails(
    ...
    const scheduledAssessment = await ScheduledAssessment.findById(
      candidateAssessment.scheduledAssessmentId
    );
    ...
    console.error(
      "[ASSESSMENT NOTIFICATIONS] Error creating submission notification:",
      error
    );
  }
}
...
async function createAssessmentExpiryWarningNotification(
  scheduledAssessment,
  hoursUntilExpiry
) {
  try {
    ...
    console.error(
      "[ASSESSMENT NOTIFICATIONS] Error creating expiry warning:",
      error
    );
  }
}
...
if (mongoose.connection.readyState === 1) {
  ...
  runAssessmentReminderJob().catch((err) =>
    console.error("[ASSESSMENT REMINDERS] Initial run error:", err)
  );
} else {
...
async function cleanupDuplicateExpiredNotifications() {
  try {
    ...
    for (const [key, notifications] of Object.entries(
      notificationsByAssessment
    )) {
