...
const updateSubscriptionPlan = async (req, res) => {
  ...
  try {
    ...
    const oldPrice = customerSubscription.price;
    const oldStatus = customerSubscription.status;
    const oldRazorpaySubscriptionId = customerSubscription.razorpaySubscriptionId;
    ...
    try {
      const action = oldPrice < finalPrice ? 'upgrade' : (oldPrice > finalPrice ? 'downgrade' : 'switched');
      await SubscriptionHistory.create({
        tenantId: customerSubscription.tenantId,
        ownerId: customerSubscription.ownerId,
        oldPlanId: oldPlanId,
