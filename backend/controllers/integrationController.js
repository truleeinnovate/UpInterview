...
const createLog = async (logData) => {
  try {
    const log = new IntegrationLog({
      ...
      requestBody: logData.requestBody ? JSON.stringify(logData.requestBody) : '',
      responseStatusCode: logData.responseStatusCode || 200,
      responseError: logData.responseError ? JSON.stringify(logData.responseError) : null,
...
exports.makeApiCall = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (integration.authentication.type === 'api_key') {
      headers[integration.authentication.apiKey.header] = integration.authentication.apiKey.value;
