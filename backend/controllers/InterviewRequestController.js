...
exports.getInterviewRequests = async (req, res) => {
  try {
    ...
    if (!contact) {
      return res
        .status(404)
        .json({ message: "Contact not found for this ownerId" });
    }
...
exports.acceptInterviewRequest = async (req, res) => {
  ...
  try {
    ...
    await InterviewRequest.deleteOne({ _id: requestId, roundId }).session(
      session
    );
    ...
    res
      .status(200)
      .json({ message: "Interview request accepted and deleted successfully" });
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
    console.error(error);
