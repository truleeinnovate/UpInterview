  ...
  try {
    ...
    if (!planDetails || !userDetails || !status) {
      return res
        .status(400)
        .json({
          message: "Missing required details cretateSubscriptionControllers.",
        });
    }
    ...
    if (existingSubscription) {
      ...
      // const plan = await SubscriptionPlan.findById(subscriptionPlanId);
      const existingInvoice = await Invoicemodels.findOne({
        ownerId: userDetails.ownerId,
      });
      if (existingInvoice) {
        existingInvoice.subscriptionId = subscriptionPlanId;
...
const updateCustomerSubscriptionControllers = async (req, res) => {
  ...
  try {
    const { planDetails, cardDetails, totalPaid, status, transactionId } =
      req.body;
    ...
    const discount = parseInt(
      cardDetails.membershipType === "monthly"
        ? planDetails.monthlyDiscount || 0
        : planDetails.annualDiscount || 0
    );
    ...
    if (status === "paid") {
      ...
      if (totalPaid >= pricing - discount) {
        ...
        if (subscription) {
          ...
          return res
            .status(404)
            .json({ message: "Subscription not found for user." });
        }
      } else {
        ...
        return res
          .status(400)
          .json({ message: "Insufficient payment. Payment failed." });
      }
    } else {
...
const getBasedTentIdCustomerSubscription = async (req, res) => {
  try {
    ...
    const customerSubscriptions = await CustomerSubscription.find({
      ownerId: ownerId,
    }).lean();
    ...
    const subscriptionPlanIds = customerSubscriptions.map(
    ...
    const subscriptionsWithPlanNames = customerSubscriptions.map((sub) => {
      const plan = subscriptionPlans.find(
        (plan) => String(plan._id) === String(sub.subscriptionPlanId)
      );
...
module.exports = {
  createSubscriptionControllers,
  updateCustomerSubscriptionControllers,
  getAllCustomerSubscription,
  getBasedTentIdCustomerSubscription,
};
