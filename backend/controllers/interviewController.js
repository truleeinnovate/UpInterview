...
const { validateInterviewRound } = require("../validations/interviewValidations.js");
// v1.0.2 <-----------------------------------------
// ------------------------------v1.0.0 >
...
//     // Only check candidate when not just updating status
//     if (!updatingInterviewStatus) {
...
//       ? await InterviewTemplate.findById(templateId)
//       : null;
...
//     // Update interview
//     let interview = await Interview.findByIdAndUpdate(id, interviewData, {
//       new: true,
//     });
//     if (!interview) {
//       return res.status(404).json({ message: "Interview not found" });
...
//     // Handle rounds/questions only if not just updating status
//     if (!updatingInterviewStatus) {
//       const position = await Position.findById(positionId);
//       let roundsToSave = [];
...
//       if (roundsToSave.length > 0) {
//         // Clear old rounds & questions before replacing
//         await InterviewRounds.deleteMany({ interviewId: interview._id });
//         await interviewQuestions.deleteMany({ interviewId: interview._id });
...
//             const validQuestions = round.questions.every(
//               (q) => q && (q.questionId || q._id) && (q.snapshot || q.question)
//             );
