...
router.post("/test-task-reminder", async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message:
        "Task reminder job executed successfully. Check console logs for details.",
    });
  } catch (error) {
...
router.post("/test-interview-reminder", async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message:
        "Interview reminder job executed successfully. Check console logs for details.",
    });
  } catch (error) {
...
router.post("/test-assessment-reminder", async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message:
        "Assessment reminder job executed successfully. Check console logs for details.",
    });
  } catch (error) {
...
router.post("/test-scheduled-assessment", async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message:
        "Test scheduled assessment notification created. Check PushNotifications collection and console logs.",
    });
  } catch (error) {
    console.error(
      "[TEST] Error creating test scheduled assessment notification:",
      error
    );
    res.status(500).json({
      success: false,
...
router.post("/test-assessment-creation", async (req, res) => {
  try {
    ...
    await createAssessmentCreatedNotification(
      mockAssessment,
      mockAssessment.ownerId
    );

    res.status(200).json({
      success: true,
      message:
        "Test assessment creation notification created. Check PushNotifications collection.",
    });
  } catch (error) {
